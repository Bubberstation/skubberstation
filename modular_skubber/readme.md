# The modularization handbook - Skubber style, v0.1
...
**If it's a bubbercode-specific tweak or bugfix, first course of action should be an attempt to discuss and PR it upstream, instead of needlessly modularizing it here.**

The best practice for modularization involves making a **`modular_skubber/`** version of whatever file you may be working on and copying that path exactly for ease of finding it. For example, if your file is **`code/game/objects/items/crab17.dm`** then you would make your new file in **`modular_skubber/code/game/objects/items/crab17.dm`**. Sometimes the folders you need may not exist already, it is perfectly fine to make new folders in order to ensure your modular file goes in the same file path as it does on Bubber just with **`modular_skubber/`** as the first folder.

New files go in **`modular_skubber/`** as well, go with your heart as to which file to put them in if there are no exact file matches on Bubber, a personal suggestion would be to look at where _similar_ files go.
...
DO NOT CHANGE BUBBER MAPS, THEY ARE HELD TO THE SAME STANDARD AS ICONS. USE THE ABOVE TO MAKE MAP EDITS.
...
- **_Example:_** You're adding a new lavaland mob.
  ...
  First of all you create your modular folder. E.g. `modular_skubber/code/modules/mob/living/basic/lavaland`

  And then you'd want to create new files and folders for each component. E.g. `modular_skubber/sound/items/weapons` for sound files and `modular_skubber/icons/mob/simple/lavaland` for any icon files.

  After doing this, you'll want to set your references within the code.

  ```byond
    /mob/living/basic/mining/new_mob
      icon = 'modular_skubber/icons/mob/simple/lavaland/lavaland_monsters.dmi'
      icon_state = "dead_1"
      sound = 'modular_skubber/sound/items/weapons/bite.ogg'
  ```
...
- Any additional clothing icon files you add MUST go into the existing files in either `modular_skubber/icons/mob/clothing` for on_mob sprites or `modular_skubber/icons/obj/clothing` for in-hand sprites.
...
In general they are a good idea and encouraged whenever it is possible to do so. However this is not a hard rule, and sometimes Skubber edits are preferable. Just try to use your common sense about it.

For example: please do not copy paste an entire BUBBER proc into a modular override, make one small change, and then bill it as 'fully modular'. These procs are an absolute nightmare to maintain because once something changes upstream you have to update the overridden proc.
...
Performance should also be considered when you are overriding a hot proc (like Life() for example), as each additional call adds overhead. Skubber edits are much more performant in those cases. For most procs this won't be something you have to think about, though.
...
- **Addition:**
  ...
  ```byond
  // SKUBBER EDIT - ADDITION - START - SHUTTLE_TOGGLE
  var/adminEmergencyNoRecall = FALSE
  var/lastMode = SHUTTLE_IDLE
  var/lastCallTime = 6000
  // SKUBBER EDIT - ADDITION - END
  ```

- **Removal:**

  ```byond
  // SKUBBER EDIT - REMOVAL - START - SHUTTLE_TOGGLE
  /*
  for(var/obj/docking_port/stationary/S in stationary)
    if(S.id = id)
      return S
  */
  // SKUBBER EDIT - REMOVAL - END
  ...
  ```byond
  // SKUBBER EDIT - REMOVAL - START - SHUTTLE_TOGGLE - (Moved to modular_skubber/shuttle_toggle/randomverbs.dm)
  /*
...
- **Change:**
  ...
  ```byond
  // SKUBBER EDIT - CHANGE - START - SHUTTLE_TOGGLE
  if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE, SHUTTLE_DISABLED)
  // SKUBBER EDIT - CHANGE - END
      return 1
  ```
...
That folder is **`code/__DEFINES/~~~skubber_defines`**, in which you can add them to the existing files, or create those files as necessary.
...
ALL of the tgui files are located in `/tgui/packages/tgui/interfaces` and its subdirectories; there is no specific folder for Skubber UIs.
...
You can do both `// SKUBBER EDIT` and `/* SKUBBER EDIT */`, though in some cases you may have to use one over the other.
...
<Button
	...
	icon="rat" // SKUBBER EDIT - ADDITION
</Button>
```
...
```js
// THIS IS A SKUBBER UI FILE
```

This way they are easily identifiable as modular TGUI .tsx/.jsx files. You do not have to do anything further, and there will never be any need for a Skubber edit comment in a modular TGUI file.
