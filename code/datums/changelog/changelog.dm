...
/datum/changelog/ui_interact(mob/user, datum/tgui/ui)
	...
	if (!ui)
		ui = new(user, src, "SkubberChangelog") // SKUBBER EDIT - CHANGE - SKUBBER CHANGELOG - ORIGINAL: ui = new(user, src, "BubberChangelog") // BUBBER EDIT CHANGE: Changelog 2: Original: ui = new(user, src, "Changelog")
		ui.open()
...
/datum/changelog/ui_static_data()
	...
	var/list/skubber_files = flist("html/changelogs/skubber_archive/") // SKUBBER EDIT - ADDITION - SKUBBER CHANGELOG
	// BUBBER EDIT ADDITION END: Changelog 2

	for(var/archive_file in sort_list(tg_files |= bubber_files |= skubber_files)) // SKUBBER EDIT - CHANGE - SKUBBER CHANGELOG - ORIGINAL: for(var/archive_file in sort_list(tg_files |= bubber_files)) // BUBBER EDIT CHANGE: Changelog 2: Original: for(var/archive_file in sort_list(flist("html/changelogs/archive/")))
