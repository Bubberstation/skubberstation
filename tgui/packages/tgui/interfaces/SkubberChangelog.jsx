...
const ChangelogList = (props) => {
  const { contents, skubberContents } = props;

  const combinedDates = {};
  Object.assign(
    combinedDates,
    typeof contents === 'object' ? contents : {},
    typeof skubberContents === 'object' ? skubberContents : {},
  );
  ...
  return Object.keys(combinedDates)
    ...
    .map((date) => (
      <Section key={date} title={dateformat(date, 'd mmmm yyyy', true)} pb={1}>
        <Box ml={3}>
          {skubberContents[date] && (
            <Section mb={-2}>
              {Object.entries(skubberContents[date]).map(([name, changes]) => (
                <SkubberChangelogEntry
                  key={name}
                  author={name}
                  changes={changes}
                />
...
const SkubberChangelogEntry = (props) => {
  ...
  return (
    <Stack.Item mb={-1} pb={1} key={author}>
      <Box>
        <h4>
          <Image verticalAlign="bottom" src={resolveAsset('skubber_16.png')} />{' '}
          {author} changed:
        </h4>
      </Box>
      <Box ml={3} mt={-0.2}>
...
export const SkubberChangelog = (props) => {
  const { data } = useBackend();
  const { dates } = data;
  const [contents, setContents] = useState('');
  const [skubberContents, setSkubberContents] = useState('');
  const [selectedDate, setSelectedDate] = useState(dates[0]);
  const [selectedDateIndex, setSelectedDateIndex] = useState(0);

  useEffect(() => {
    setContents('Loading changelog data...');
    setSkubberContents('Loading changelog data...');
  ...
  function getData(date, attemptNumber = 1) {
    ...
    Promise.all([
      ...
      fetch(resolveAsset(`skubber_${date}.yml`)),
    ]).then(async (links) => {
      const result = await links[0].text();
      const skubberResult = await links[1].text();
      ...
      if (links[0].status !== 200 && links[1].status !== 200) {
        ...
        if (links[1].status === 200) {
          setSkubberContents(
            yaml.load(skubberResult, { schema: yaml.CORE_SCHEMA }),
          );
        }
      }
    });
  ...
  const header = (
    <Section>
      <h1>Skubberstation</h1>
      <p>
        <b>Thanks to: </b>
        Bubberstation, /tg/station 13, Effigy, Stellar Haven, Baystation 12,
      ...
      <p>
        ...
        <a href="https://github.com/Bubberstation/Skubberstation/pulse/monthly">
          here
        </a>
        .
      </p>
      <p>
        ...
        <a href="https://discord.gg/3sn27NGVpn">here</a>!
      </p>
      <DateDropdown
        dates={dates}
  ...
  const footer = (
    <Section>
      ...
      />
      <h2>Licenses</h2>
      <Section title="Skubberstation">
        <p>
          {'All code is licensed under '}
          <a href="https://www.gnu.org/licenses/agpl-3.0.html">GNU AGPL v3</a>.
          {' See '}
          <a href="https://github.com/bubberstation/skubberstation/blob/master/LICENSE">
            LICENSE
          </a>{' '}
          for more details.
        </p>
      ...
      <Section title="Goonstation SS13">
        ...
        <p>
          Skubberstation and /tg/station 13 are thankful to the GoonStation 13
  ...
  return (
    <Window title="Changelog" width={730} height={700}>
      <Window.Content scrollable>
        {header}
        <ChangelogList contents={contents} skubberContents={skubberContents} />
        {footer}
      </Window.Content>
    </Window>
  );
